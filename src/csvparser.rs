...
fn extract_cash(cashline: &str) -> Result<crate::Currency, String> {
    ...
    let to_decimal = |v: f64| -> Result<Decimal, String> {
        Decimal::from_str(&v.to_string())
            .map_err(|e| format!("error converting {v} to Decimal: {e}"))
    };
...
fn process_tax_consolidated_data(
    ...
    match state {
        ...
        ParsingState::InterestsEUR(s) | ParsingState::InterestsPLN(s) => {
            ...
            let ltaxes: Vec<crate::Currency> = lincomes.iter().map(|i| i.derive(Decimal::ZERO)).collect();
...
mod tests {
    ...
    fn test_extract_cash() -> Result<(), String> {
        ...
        assert_eq!(extract_cash("6 000€"), Ok(crate::Currency::EUR(dec!(6000.00))));
        assert_eq!(extract_cash("600,34€"), Ok(crate::Currency::EUR(dec!(600.34))));

        assert_eq!(extract_cash("€840.03"), Ok(crate::Currency::EUR(dec!(840.03))));
        assert_eq!(extract_cash("€0.01"), Ok(crate::Currency::EUR(dec!(0.01))));
        assert_eq!(extract_cash("€440"), Ok(crate::Currency::EUR(dec!(440.0))));

        assert_eq!(extract_cash("1,06 PLN"), Ok(crate::Currency::PLN(dec!(1.06))));
        assert_eq!(
        ...
        assert_eq!(extract_cash("-$0.51"), Ok(crate::Currency::USD(dec!(-0.51))));

        assert_eq!(extract_cash("63,28$"), Ok(crate::Currency::USD(dec!(63.28))));
